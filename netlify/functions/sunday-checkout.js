...
const day=d=>new Intl.DateTimeFormat('en-US',{weekday:'short',month:'short',day:'numeric',timeZone:'UTC'}).format(new Date(d+'T00:00:00Z'));
...
exports.handler=async event=>{
  ...
  if(!DATES.includes(date)||date<today)return json(400,{error:'bad_date'});
  ...
  try{
    ...
    const hold=await sb('/bookings',{method:'POST',prefer:'return=representation',body:{kind:'pass',item_id:'sunday-funday',item_name:'Sunday Funday',price_cents:3000,athlete_name:athlete,athlete_age:String(input.age||'').trim()||null,sport:String(input.sport||'').trim()||null,parent_name:String(input.parent||'').trim()||null,contact,sms_opt_in:input.smsOptIn===true,status:'hold',hold_expires_at:new Date(Date.now()+HOLD_MINUTES*60000).toISOString()}});
    if(hold.status===409)return json(409,{error:'full'});
    if(!hold.ok||!hold.data?.[0])return json(502,{error:'db'});
    const booking=hold.data[0];
    const days=await sb('/booking_days',{method:'POST',body:[{booking_id:booking.id,session_date:date}]});
    ...
    const origin=process.env.URL||`https://${event.headers.host}`,when=`${day(date)} · 9:00–11:00 AM`;
    const params=new URLSearchParams({mode:'payment',allow_promotion_codes:'true',expires_at:String(Math.floor(Date.now()/1000)+1800),'line_items[0][quantity]':'1','line_items[0][price_data][currency]':'usd','line_items[0][price_data][unit_amount]':'3000','line_items[0][price_data][product_data][name]':`Sunday Funday — ${when}`,'line_items[0][price_data][product_data][description]':`Athlete: ${athlete}`,'success_url':`${origin}/book/?paid=1&sid={CHECKOUT_SESSION_ID}`,'cancel_url':`${origin}/sunday-funday?cancelled=1&bid=${booking.id}`,'metadata[booking_id]':booking.id,'payment_intent_data[description]':`Sunday Funday (${when}) — ${athlete}`});
