...
export default function NevinsPitching(){
  ...
  const dates=useMemo(()=>{const out=[],today=new Intl.DateTimeFormat('en-CA',{timeZone:'America/New_York'}).format(new Date()),d=new Date(`${today}T12:00:00Z`);while(out.length<10){if(d.getUTCDay()===4)out.push(iso(d));d.setUTCDate(d.getUTCDate()+1)}return out},[]);
  ...
  const total=selected.length*25;
  ...
  return <main className={s.page}>
    ...
    <section className={s.hero}><div><p className={s.kicker}>Thursdays · 6:00–7:00 PM</p><h1>Nevin's <em>Pitching</em></h1><p className={s.lead}>Pitch with purpose.</p><div className={s.facts}><strong>$25<small>per player</small></strong><span>Former White Sox pitcher</span><span>Limited spots</span></div><a className={s.cta} href="#reserve">Reserve a spot ↓</a></div><div className={s.image}><img src="/assets/pitching-clinic.jpg?v=20260923" alt="Nevin Griffith pitching clinic flyer"/></div></section>
