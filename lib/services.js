...
const PASSES = [
  {
    id: 'afterschool', name: 'After-School Training', cents: 2000,
    unit: 'Mon $25 · Tue–Fri $20', allowedDays: [1, 2, 3, 4, 5],
    desc: 'Coach-led player development after school until 5:00 PM. Select one or more weekdays.',
  },
  {
    id: 'diamond-skills', name: 'Evening Skills Training', cents: 2000,
...
function programPriceCents(program, dates) {
  if (program.id === 'afterschool') {
    return dates.reduce((total, iso) => total + (dayOfWeek(iso) === 1 ? 2500 : 2000), 0);
  }
...
module.exports = {
  ...
  fmtTime, overlaps, isIsoDate, isWeekday, dayOfWeek, isProgramDate,
